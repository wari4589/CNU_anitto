...
async function loadGameContext() {
  App.isAdmin = App.profile?.is_admin || false;
...
async function tryLogin() {
  ...
  await loadGameContext();
  App.isAdmin ? enterAdminApp() : enterUserApp();
}
...
/* ─── 관리자 로그인 ──────────────────────────────────────── */
// Legacy 보관본에서는 관리자 로그인이 비활성화되어 있습니다.
const LEGACY_ADMIN_LOGIN_ENABLED = false;

async function tryAdminLogin() {
  const pw = document.getElementById('a-pw')?.value?.trim();
  if (!pw) { setAuthError('admin-err', '비밀번호를 입력해주세요.'); return; }

  if (!LEGACY_ADMIN_LOGIN_ENABLED) {
    setAuthError('admin-err', 'Legacy 보관본에서는 관리자 로그인이 비활성화되어 있습니다.');
    return;
  }

  // 비밀번호 일치 → 관리자 모드로 진입
  App.isAdmin = true;
  App.profile = { real_name: '관리자', nickname: 'admin', is_admin: true };
    ...
    list.map((m, i) => {
      ...
      <div class="mission-item">
        ...
        ${m.completed
          ...
          : `<button class="btn-complete active" onclick="confirmCompleteMission('${m.id}','${esc(m.title)}',${m.score_reward},${m.coin_reward})">완료하기</button>`
        }
      </div>`;
    }).join('') + '</div>';
}
  ...
  openModal('confirm', {
    ...
    extra: `
      <div style="margin-bottom:16px">
        ...
        <div id="upload-preview" style="border:1px dashed var(--border2);border-radius:10px;padding:18px;text-align:center;color:var(--text3);font-size:13px;cursor:pointer" onclick="document.getElementById('photo-input').click()">
  ...
  el.innerHTML = data.map((m, i) => {
    ...
    <div class="card" style="padding:14px 16px;margin-bottom:8px">
      <div style="display:flex;align-items:flex-start;gap:10px">
        ...
        <button class="btn-chip danger" onclick="adminDeleteMission('${m.id}')">
  ...
  const actions = {
    ...
    create: {
      ...
      onConfirm: async () => {
        ...
        // season_id는 첫 번째 시즌 or 없으면 새로 생성
        let seasonId = null;
        const { data: seasons } = await ManittoDB.client.from('seasons').select('id').limit(1).single();
        if (seasons) {
          seasonId = seasons.id;
        } else {
          const { data: newSeason } = await ManittoDB.client.from('seasons').insert({ name: '기본 시즌' }).select().single();
          seasonId = newSeason?.id;
        }
        const { data, error } = await ManittoDB.adminCreateGameSession({ seasonId, name, durationDays: days });
        if (error) { showToast('오류: ' + error.message); return; }
        App.gameSession = data;
        await loadAdminStats();
  ...
  if (type === 'add-mission') {
    showModalRaw(`
      ...
      <div class="modal-btns">
        ...
        <button class="modal-btn confirm" onclick="addAdminMission()">추가하기</button>
      </div>`);
  }
}
...
function endingHTML(partLabel, title, subtitle, extra, nextLabel = '다음') {
  document.getElementById('ending-content').innerHTML = `
    <div style="animation:fadeUp .5s ease">
      ...
      ${extra}
      <button class="ending-btn" onclick="nextEndingStep()">
...
function openModal(type, opts = {}) {
  if (type === 'confirm') {
    ...
    showModalRaw(`
      ...
      <div class="modal-btns">
        <button class="modal-btn cancel" onclick="closeModal()">취소</button>
        <button class="modal-btn ${opts.btnClass || 'confirm'}" onclick="handleModalConfirm()">
          ${opts.btnText || '확인'}
        </button>
      </div>`);
  }
...
function errorState(msg) {
  return `<div style="padding:32px;text-align:center;color:var(--red)">
    ...
    <div style="font-size:14px">${msg}</div>
    <button class="btn-ghost-sm" style="margin-top:12px" onclick="location.reload()">새로고침</button>
  </div>`;
}
